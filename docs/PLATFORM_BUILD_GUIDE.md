...
本平台的建置歷經七大核心階段，記錄了從最初的空間模型確立，到極致效能調優、權限防護，以及最後的行動裝置無遮擋響應式重構與持久化安全防護。
...
flowchart TD
    ...
    H --> I[Phase 9: 水域與非人行區防呆判定機制與零基礎分規範]
```
...
### 1.9 Phase 9：水域與非人行區域防呆判定機制與「零基礎分」規範 (Non-Pedestrian Zone Guardrail)
- **目標**：徹底解決「在湖泊水域（如澄清湖、蓮池潭）或無設施荒地中間框選範圍時，系統因未檢測到道路與事故，誤給予 40 分步行保底分與 100 分交通安全滿分，導致湖面竟能拿到 28~35 分基礎分」的業務邏輯漏洞。
- **關鍵踩坑與底層成因**：
  1. **步行分數無條件保底陷阱**：原本後端空間分析演算法設定 `weighted_walk_score = 40.0` 作為初始值，當框選水面無任何道路路網時，該值未被重設，自動核發 40 分（加權 45% 貢獻 18 分）。
  2. **零事故給滿分邏輯偏差**：交通安全指數 `S_SAFETY` 以「100 - 事故扣分」計算，水域中央從無行人與車輛通行，當然 0 事故，結果被演算法直接判定為「100 分滿分安全環境」（加權 10% 貢獻 10 分）。
  3. **兩者合計產生 28 分虛假基礎分**：$18 + 10 = 28$ 分，並被評定為 `E級 (亟待改善)`，讓主管與使用者產生「湖面根本沒有路，怎麼還能評鑑並給 28 分」的強烈質疑。
- **解法與防呆邊界規則**：
  1. **全無設施空間初篩準則**：
     - 若框選區域滿足：`total_clipped_len == 0`（無 12m 道路）且 `sw_len == 0`（無實體人行道）且 `scoreable_poi_cnt == 0`（無生活設施）且 `tot_acc == 0`（無事故紀錄），系統即判定為**「非人行評估範圍（水域/未開闢區）」**。
  2. **全面歸零與狀態標記**：
     - `is_evaluable = False`，總分與各項維度分數一律強制為 `0.0`。
     - 品質等級標註為：`不適用 (非人行評估範圍)`。
  3. **前端雙重視覺反饋**：
     - 右側抽屜總分顯示為 `-- / 100 分 (不適用)`，等級徽章套用中性灰色標籤。
     - 總分卡片下方插入顯著的友善警示條：「🌊 非有效人行評估空間：所選範圍內查無道路路網、實體人行道或生活設施（如水域、湖泊），不具備人行評鑑條件，系統不發放基礎分。」
     - 匯出成果圖同步呈現 `--` 與中性標記，杜絕任何荒謬的湖水評分報告流出。
