...
本平台的建置歷經九大核心階段，記錄了從最初的空間模型確立，到極致效能調優、權限防護，以及最後的行動裝置無遮擋響應式重構、正面五級分評估改制與無道路資料防呆攔截。
...
flowchart TD
    ...
    F --> G[Phase 7: 雲端部署帳密持久化與密碼可見性控制]
    G --> H[Phase 8: 道路評估全面轉正向評分與五級分制對齊]
    H --> I[Phase 9: 無道路資料前置攔截與防呆不予分析機制]
```
...
### 1.7 Phase 7：雲端部署帳密持久化防重置機制與密碼可見性控制 (Password Persistence & Toggle)
- **目標**：解決「每次系統更新或重新部署後，管理員帳號密碼都會被重置回預設值」的重大維運痛點，並在全站所有輸入密碼介面增加「顯示/隱藏密碼」切換（👁️ / 🙈）功能。
- **關鍵踩坑與底層原因**：
  1. **`.gitignore` 誤略 `.db`**：先前 `.gitignore` 設定了 `*.db`，導致 `server/admin.db` 從未納入 Git 版本追蹤。當代碼推送到 GitHub、Render 觸發自動重新構建（Redeploy）時，Render 的暫態環境（Ephemeral Filesystem）拉取乾淨代碼，此時根本不存在 `admin.db`。
  2. **`init_db()` 自動重置陷阱**：後端 `auth_db.py` 啟動時檢測 `SELECT COUNT(*) FROM users`，因無資料庫檔案檢測為 0，便強制建立預設帳號（`admin / admin888`），導致管理員修改過的密碼及新增的同仁帳號全部蒸發！
- **解法與架構創新**：
  1. **雙軌持久化同步架構 (Dual Persistence Architecture)**：
     - **自動種子匯出 (`export_users_seed()`)**：每當「新增使用者」、「刪除使用者」、「修改密碼」或系統首次初始化成功時，即刻將現有帳號、PBKDF2 密碼雜湊與 Salt 序列化儲存至 `server/users_seed.json`。
     - **防禦性自動還原 (`restore_users_from_seed()`)**：當伺服器因雲端重新構建而面對全新或空白的 SQLite 資料庫時，`init_db()` 優先讀取 `users_seed.json` 並完整還原所有同仁帳號與密碼雜湊，確保部署後密碼永久有效。
     - **環境變數備援**：亦支援讀取雲端環境變數 `ADMIN_PASSWORD` 作為最高優先初始憑證。
  2. **Git 白名單追蹤放行**：
     - 在 `.gitignore` 中明確聲明 `!server/admin.db` 與 `!server/users_seed.json`，確保本機與雲端維護的最新資料庫與種子檔能順暢納入 Git 提交。
  3. **密碼輸入可見性切換 (Password Visibility Toggle)**：
     - 於前端「系統登入彈窗」、「新增使用者」及「個人密碼修改」等三處密碼輸入框建立 `.pwd-input-container`，右側配置無障礙切換按鈕 `👁️` / `🙈`。
     - 一鍵切換 `type="password"` 與 `type="text"`，杜絕公務人員手民誤植難以核對的困擾，並保持手機與電腦端統一且舒適的觸控體驗。

---

### 1.8 Phase 8：道路人本環境全面改制「大眾正面評分」與五級分制對齊 (Citizen-Centric Walkability Scoring)
- **目標**：解決「地圖道路點擊氣泡與屬性抽屜原顯示工務局改善急迫度扣分（I_TOTAL，分數越高代表路況越差、越急迫，70~86分為紅色 Tier 1），造成民眾誤解為高分卻是劣質環境」的認知矛盾，全面轉化為直觀的大眾正面步行優良度評估。
- **關鍵踩坑與業務邏輯痛點**：
  - **認知衝突**：民眾習慣「分數越高越好」，看到「86分」會直覺認為是優秀道路，但工務指標中 86 分卻是全高雄市最急迫改善的劣化路段（紅色 Tier 1）。
  - **與生活圈分析評分標準脫鉤**：生活圈手繪框選分析採用國土署 2.0 V2 正向五級分制（A~E 級，A 級最優），而道路點擊卻顯示負向扣分制，兩種標準在同一平台上造成市民混淆。
- **解法與雙軌呈現架構**：
  1. **正面分數轉換模型**：
     $$\text{WalkScore} = \max(0, \min(100, \text{round}(100 - I\_TOTAL, 1)))$$
     將原本 $16 \sim 86$ 分的工務急迫扣分，轉換為 $14 \sim 84$ 分的正向人本通行良好度總分。
  2. **統一五級分制標準（國土署 2.0 V2）**：
     - **A級 (≥80分)**：優良環境 (綠色 `#16a34a`)，鋪面平整且淨寬充裕。
     - **B級 (65~79.9分)**：良好通行 (青綠 `#0d9488`)，步行路網連貫。
     - **C級 (50~64.9分)**：普通環境 (黃橙 `#d97706`)，具備局部改善缺口。
     - **D級 (35~49.9分)**：待改善 (橘色 `#ea580c`)，人行設施缺乏或事故頻率偏高。
     - **E級 (<35分)**：亟待改善 (紅色 `#dc2626`)，步行障礙嚴重亟需工務優先介入。
  3. **地圖顏色與圖例無縫咬合**：
     - MapLibre 線段顏色以 `I_TOTAL` 的階梯閾值渲染（`<35` 綠色、`35~50` 黃色、`50~70` 橘色、`>=70` 紅色），剛好完全對齊正向評分高到低的色彩光譜。
     - 於左側圖例面板正式加入「12M+ 道路步行良好度五級分級」色塊說明，市民比對地圖一目了然。
  4. **大眾視角與工務專案雙軌並存**：
     - 在地圖 Pop up 與右側抽屜（`#right-drawer`）中，大字強調「步行良好度評分」與「等級徽章」。
     - 同步於次要區塊保留「工務改善優先度（Tier 1~4）」、「全市排名」及「急迫扣分指標」，滿足工務工程同仁查閱工程排序的需求。
  5. **步行環境品質指標 ($I_{\text{sidewalk}}$) 兼納正向加分與負向扣分**：
     - 確立國土署正面評估版 V2 精神：人行道評分並非單純瑕疵扣分，而是**正向加分**與**負向扣分**之綜合評定：
       - **➕ 正向加分項目**：充足淨寬（$\ge 1.5\text{m}$ 基礎合規、$\ge 2.0\text{m}$ 寬裕加分）、無障礙斜坡道 (`SW_RAMP`)、連續植栽帶與行道樹遮蔭 (`SW_TREE`)、獨立公共設施帶集中收整箱桿 (`SWD_WTH`)、騎樓無高低落差整平 (`AC_EVEN`)。
       - **➖ 負向扣分項目**：淨寬不足（$<1.5\text{m}$，尤其 $<0.9\text{m}$ 輪椅無法通過者加重扣分）、鋪面破損老化 (`SW_BRKRAT`, `SW_BK_B/L`)、固定路障占用淨寬、動線中斷斷點。
     - 於系統說明面板完整呈現雙欄對照表，使公務同仁與市民充分理解評分之科學依據。

---

### 1.9 Phase 9：無道路資料前置攔截與防呆不予分析機制 (No Road Guardrail)
- **目標**：確立「框選分析範圍內沒有道路則不予分析」原則。當框選範圍內查無道路與人行道時，系統即時拒絕分析，避免產生不合理的基礎分數。文案純粹客觀，統一標示「範圍內無道路與人行道資料」。
- **關鍵踩坑與底層成因**：
  1. **步行分數無條件保底陷阱**：原本後端演算法設定 `weighted_walk_score = 40.0`，當框選無道路區域時未被重設，自動核發 40 分（加權 45% 貢獻 18 分）。
  2. **零事故給滿分邏輯偏差**：交通安全指數以「100 - 事故扣分」計算，無道路處無事故，被演算法判定為 100 分滿分安全（加權 10% 貢獻 10 分）。
  3. **兩者合計產生 28 分虛假基礎分**：$18 + 10 = 28$ 分，導致無路區域仍能計算出分數。
- **解法與防呆邊界規則**：
  1. **前置道路快速初篩**：
     - 在後端空間運算之初，優先以空間索引檢核 `road_priority`（12公尺以上道路）與 `sidewalk`（實體人行步道）。
     - 若兩者於框選多邊形內皆無交集（長度均為 0），代表「範圍內無道路與人行道資料」。
  2. **不予分析並立即返回錯誤**：
     - 後端即時返回 HTTP 400：`{"valid": false, "has_roads": false, "error": "範圍內無道路與人行道資料，不予分析"}`，不執行任何後續指標運算，節省運算效能。
  3. **前端一致性反饋與抽屜關閉**：
     - 前端彈出輕量提示：「**範圍內無道路與人行道資料，不予分析**」。
     - 抽屜載入動畫自動關閉，不核發基礎分、不呈現評估卡片，完全杜絕無道路區域產出分數的現象。
  4. **框選幾何即時清除與雙軌控制按鈕 (Draw State & Geometry Cleanup)**：
     - **痛點排查**：先前「結束框選 (`stopDrawing`)」代碼中保留了完成幾何 (`finishedFeature`)，且框選面板未配置清除按鈕，導致使用者結束框選後圖形仍懸浮在圖面上，被迫切換至隔壁分頁（GIS 量測）才能清除。
     - **徹底修復**：
       1. `drawTool.stopDrawing()` 改為直接調用 `this.clear()`，點擊「結束框選」時徹底移除地圖上的框選範圍、取消高亮並收合抽屜。
       2. 當分析中止或不予分析（`catch` 捕捉錯誤）時，前端自動調用 `this.clear()`，杜絕無效幾何殘留。
       3. 於「手繪區域框選分析」面板新增專屬「🗑️ 清除框選」按鈕，與「結束框選」成對佈局，達到全站一致的操作直覺。
       4. 側欄分頁切換加入防呆：離開「生活圈分析」分頁時自動中止未完成的繪圖與選點手勢，行動端同步關閉操作膠囊。
...
| **身分驗證與資料庫** | **SQLite (WAL 模式) + PBKDF2 + users_seed.json 雙軌持久化** | • 單一檔案資料庫，隨代碼一同備份，零連線管理與連接池洩漏風險。<br>• 安全性達銀行級標準（隨機鹽值 + 10 萬次雜湊疊代）。<br>• 搭配自動 JSON 種子檔匯出與重啟還原機制，徹底免除雲端容器重新構建重置帳密的風險。 | **PostgreSQL / MySQL**：過度設計，維護連線池與備份負擔大。 |
...
## 5. 帳號密碼持久化與密碼顯示切換防護
1. 帳號雙軌持久化：
   - 後端使用 SQLite 資料庫，每次新增/刪除帳號或修改密碼時，自動序列化導出 `server/users_seed.json`。
   - `init_db()` 初始化時若偵測資料表為空（雲端容器重新建置拉取乾淨環境），優先從 `users_seed.json` 完整還原所有同仁帳號與密碼雜湊，杜絕部署後帳密重置。
   - `.gitignore` 白名單放行 `!server/admin.db` 與 `!server/users_seed.json`。
2. 密碼輸入可見性切換：
   - 所有密碼輸入框均配置眼睛切換按鈕 (👁️ / 🙈)，支援一鍵切換 `password` 與 `text` 類型，並具備優良的觸控無障礙點擊體驗。
